...
function generateCalendar(date = currentDate) {
    ...
    for (let i = 1; i <= lastDate; i++) {
        ...
        div.addEventListener("click", function () {
            ...
            if (tabsData[currentTab].selectedDates[monthKey].includes(i)) {
                removeCheckmark(div);
                tabsData[currentTab].selectedDates[monthKey] = tabsData[currentTab].selectedDates[monthKey].filter(
                    (day) => day !== i
                );
            } else {
                addCheckmark(div);
...
function updateThisMonthCount() {
    ...
    let markedDays = tabsData[currentTab].selectedDates[monthKey] ? tabsData[currentTab].selectedDates[monthKey].length : 0;
